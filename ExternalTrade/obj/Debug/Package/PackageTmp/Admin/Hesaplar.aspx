<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="Hesaplar.aspx.cs" Inherits="ExternalTrade.Admin.Hesaplar" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="head" runat="server">
    <script>
        function successAlert() {
            ...
            }).then(function () { window.location = 'Hesaplar.aspx' });
        }
        function errorAlert() {
            swal({
    ...
    <style type="text/css">
     ...
     <div class="row" style:"width:100%;">
        <%-- <h1>SATIŞ ONAYI BEKLEYEN TEKLİFLER DETAY</h1>--%>
        <div class="col-11" style:"width:100%;"><br />
          ...
          <div>
               <div>
                    <asp:Button ID="Button1" runat="server" Text="ÇEK" CssClass="btn btn-primary" Height="40px" OnClick="Button1_Click"/>
                   <asp:Button ID="btnGuncelle" runat="server" Text="GÖNDER" CssClass="btn btn-success" OnClick="btnGuncelle_Click" Height="40px" />
                    <asp:Button ID="btnHepsi" runat="server" Text="Hepsini Gönder" CssClass="btn btn-warning" Height="40px" OnClick="btnHepsi_Click" />
              </div>    
        <dx:ASPxGridViewExporter ID="ASPxGridViewExporter1" runat="server">
        </dx:ASPxGridViewExporter>
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ExternalTradeDBConnectionString %>" SelectCommand="select ROW_NUMBER() OVER(order by e.SiparisNo) as Sira,e.SiparisNo,e.TeklifNo,e.Temsilci,e.MusteriIsmi,e.Kita,e.Ulke,e.Vade,e.KalkisLimani,e.VarisLimani,e.TeklifTipi,e.KonteynerBulkSecimi,e.EkipmanTuru,e.SiparistekiKonteynerSayisi,e.Urun,e.UrunAnaGrup,
...
from EditOrders e left outer join Orders o on e.SiparisNo=o.SiparisNo and o.YediyuzluKod=e.YediyuzluKod
 where   o.SiparisNo is not null and o.YediyuzluKod is not null and o.Durum='SATISA DONDU' and e.MusteriKodu not in(select MusteriKodu from DepartmentCustomers) and Active=0
...
e.TorbaTipi,e.TorbaAgirligi,e.BBTipi,e.BBAgirligi,e.Palet,e.UrunFiyat,e.Ambalaj,e.AraNakliyeTON,e.DepolamaTON,e.GumruklemeTON,e.ArmatorLokali,e.LimanIcDolumBedeliTON,e.Inspection,e.Komisyon,e.FobMaliyet,e.FobSatisFiyati,e.NavlunTON,e.SigortaTON,e.KalemMaliyeti,e.KalemSatisFiyati,e.buUrununSiparisTonaji" UpdateCommand="OperationRealite" UpdateCommandType="StoredProcedure" OnSelecting="SqlDataSource1_Selecting">
            <UpdateParameters>
